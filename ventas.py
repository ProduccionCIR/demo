...
class ModuloVentas:
    ...
    def generar_pdf_factura(self, datos, es_offshore=False):
        ...
        # --- DISEÑO MONOCROMÁTICO (BLANCO Y NEGRO PURO) ---
        html_template = """
        <!DOCTYPE html>
        <html lang="es">
        <head>
                ...
                <tbody>
                    ...
                    <tr>
                        <td class="font-bold">
                            ...
                            <br><small style="font-weight: normal; color: #555;">CB: {{ item.codigo_barra }}</small>
                            {% endif %}
                        </td>
                        <td>{{ item.nombre }}</td>
    ...
    def formulario_venta(self, es_offshore=False):
        ...
        if not es_offshore:
            ...
            if productos_raw:
                ...
                # Mapeo defensivo de columnas a Mayúsculas para evitar problemas de case-sensitivity
                df_p.columns = [c.upper() for c in df_p.columns]
                col_ref, col_des, col_can, col_cos = "REFERENCIA", "DESCRIPCION", "CANTIDAD", "COSTO UNIT"
                
                # Sincronización robusta del código de barra
                if 'COD_BARRA' not in df_p.columns:
                ...
                # --- VISUALIZACIÓN DETALLADA EN EL SELECTOR ---
                def build_search_string(row):
                    cb = f"[{row['COD_BARRA']}]" if row['COD_BARRA'] else "[Sin Código]"
                    ref = row['REFERENCIA'] if row['REFERENCIA'] else "Sin Referencia"
                    desc = row['DESCRIPCION'] if row['DESCRIPCION'] else "Sin Descripción"
                    return f"{cb} {ref} - {desc}"

                df_p['search'] = df_p.apply(build_search_string, axis=1)

                p_sel = st.selectbox("🔍 Buscar por Código de Barra, Referencia o Descripción", ["--"] + df_p['search'].tolist(), key=f"sel_prod_{tipo_str}")

                if p_sel != "--":
                    it = df_p[df_p['search'] == p_sel].iloc[0]
                    
                    # Extracción mapeada con llaves dinámicas para evitar errores de mayúsculas/minúsculas
                    dict_it = {k.upper(): v for k, v in it.to_dict().items()}
                    
                    raw_peso = dict_it.get('PESO', dict_it.get('PESO_KG', 0.0))
                    raw_cubic = dict_it.get('CUBICAJE', dict_it.get('CBM', 0.0))
                    raw_costo = dict_it.get(col_cos, 0.0)
                    raw_cb = dict_it.get('COD_BARRA', '')

                    with st.container(border=True):
                        st.markdown(f"**Producto Seleccionado:** {dict_it.get(col_des)}")
                        c1, c2, c3, c4, c5 = st.columns(5)
                        
                        costo_base = float(raw_costo) if not pd.isna(raw_costo) else 0.0
                        
                        pr = c1.number_input("Precio Venta", value=costo_base, format="%.2f", key=f"pr_{tipo_str}")
                        ct = c2.number_input("Cantidad (Piezas)", min_value=1, value=1, key=f"ct_{tipo_str}")
                        bu = c3.number_input("Bultos", min_value=1, value=1, key=f"bu_{tipo_str}")
                        pe = c4.number_input("Peso (KG)", min_value=0.0, value=float(raw_peso if pd.notnull(raw_peso) else 0.0), format="%.2f", key=f"pe_{tipo_str}")
                        cb = c5.number_input("CBM (m³)", min_value=0.0, value=float(raw_cubic if pd.notnull(raw_cubic) else 0.0), format="%.3f", key=f"cb_{tipo_str}")

                        if st.button("Añadir al Carrito", use_container_width=True, key=f"add_{tipo_str}"):
                            nuevo_item = {
                                "id": str(dict_it[col_ref]), 
                                "nombre": str(dict_it[col_des]), 
                                "codigo_barra": raw_cb,
                                "cantidad": int(ct), 
                                "precio": float(pr), 
            ...
            # --- MODO OFFSHORE: INGRESO MANUAL ---
            st.write("### ✍️ Ingreso Manual de Productos (Offshore)")
            with st.container(border=True):
                c_ref, c_desc, c_cb = st.columns([1, 2, 1])
                ...
                manual_cb = c_cb.text_input("Código de Barra (Opcional)", key=f"man_cb_{tipo_str}")
                ...
                if st.button("Añadir al Carrito", use_container_width=True, key=f"add_{tipo_str}"):
                    if not manual_ref.strip() or not manual_desc.strip():
                        st.error("⚠️ Error: Referencia y Descripción son campos obligatorios.")
                    else:
                        nuevo_item = {
                            "id": str(manual_ref).strip().upper(), 
                            "nombre": str(manual_desc).strip(), 
                            "codigo_barra": str(manual_cb).strip(),
                            "cantidad": int(ct), 
                            "precio": float(pr), 
                            "subtotal": float(ct * pr),
                            "bultos": int(bu), 
        ...
        if carrito:
            ...
            cols_mostrar = ["id"]
            if "codigo_barra" in df_items_cart.columns:
                cols_mostrar.append("codigo_barra")
            cols_mostrar += ["nombre", "cantidad", "bultos", "precio", "subtotal"]
            
            st.dataframe(df_items_cart[cols_mostrar], hide_index=True, use_container_width=True)
            ...
            if st.button("🗑️ Vaciar Carrito", key=f"clear_cart_{tipo_str}"):
                st.session_state[f'cart_{tipo_str}'] = []
                st.rerun()
    ...
    def render_historial(self):
        ...
        for _, v in df_v.iterrows():
            ...
            with st.container(border=True):
                ...
                es_off = True if str(v.get('via_despacho', '')).lower() == 'traspaso' or 'off' in str(v.get('cliente', '')).lower() else False
                pdf_data = self.generar_pdf_factura(dict_venta, es_offshore=es_off)
                
                c2.download_button("📄 PDF", pdf_data, f"FACT_{num_visible}.pdf", key=f"pdf_h_{v['id']}", use_container_width=True)
