        ...
        if cli_sel != "--":
            with st.container(border=True):
                ...
                if prods:
                    ...
                    p_sel = st.selectbox("🔍 Buscar Producto (Referencia, Marca, Descripción)", ["--"] + df_p['search'].tolist())
                    
                    if p_sel != "--":
                        it = df_p[df_p['search'] == p_sel].iloc[0]
                        
                        # ---- CORRECCIÓN: EXTRACCIÓN USANDO PESO Y CUBICAJE DESDE LA TABLA ----
                        raw_empaque = it.get('EMPAQUE', '')
                        raw_peso = it.get('PESO', 0.0)
                        raw_cubic = it.get('CUBICAJE', 0.0)
                        raw_um = it.get('U/M', 'CAJA')
                        raw_precio = it.get('PRECIO', 0.0)

                        st.markdown("### 📋 Datos de Envío (Campos Editables / Manuales)")
                        
                        # Fila 1 de Inputs Mecánicos
                        ...
                        cubic_p3 = cc3.number_input("Cubicación Total (M3)", value=float(raw_cubic if pd.notnull(raw_cubic) else 0.0), step=0.01)
                        um = cc4.text_input("Unidad de Medida (U/M)", value=str(raw_um if pd.notnull(raw_um) else "CAJA"))

                        # Fila 2 de Inputs Comerciales
